...
Los transformadores que se incluyen en el pipeline se ajustan al conjunto de entrenamiento utilizando el método **.fit()**, lo que permite que las transformaciones se calculen basándose en los datos de entrenamiento. 
...
# Datos de entrenamiento (reemplaza 'X_train_cat' con tus datos)
X_train_cat = np.array([['A'], ['B'], ['A'], ['C'], ['B'], ['B']])
...
# Datos de prueba (reemplaza 'X_test_cat' con tus datos)
X_test_cat = np.array([['A'], ['C'], ['B']])
...
El **ColumnTransformer** es útil cuando tienes un conjunto de datos que contiene diferentes tipos de características (numéricas, categóricas, texto, etc.) y deseas aplicar diferentes transformaciones a cada tipo de característica de manera eficiente.

A continuación se muestra un ejemplo para el **caso numérico**:
...
# Datos de ejemplo con valores faltantes (reemplaza con tus datos)
...
A continuación se muestra un ejemplo para el **caso categórico**:
...
# Datos de ejemplo en forma de DataFrame (reemplaza con tus datos)
data = pd.DataFrame({
    'Column1': [1, 5, 9],
    'Column2': [2, 6, 10],
    'Column3': [3, 7, 11],
...
def custom_function(X, col1, col2, col3):
    result = (X[col1] + X[col2])/ X[col3]
    return result.values.reshape(-1, 1)

# Nombres de las columnas a operar
col1 = 'Column1'
col2 = 'Column2'
col3 = 'Column3'
...
column_transformer = ColumnTransformer(transformers=[
    ('custom', custom_transformer, [col1, col2, col3])  # Indica las columnas a transformar
])
...
# Aplicar el pipeline a los datos
new_column = pipeline.fit_transform(data)
new_column
```
...
# Crear el transformador de función personalizada
custom_transformer = FunctionTransformer(custom_function, validate=False, kw_args={
    'col1': col1,
    'col2': col2,
    'col3': col3
})
...
X_test_transformed = pipeline.transform(data)

# Crear y entrenar un modelo de ML (por ejemplo, Regresión Lineal) en los datos de entrenamiento transformados
model = LinearRegression()
model.fit(X_train_transformed, y_train)

# Realizar predicciones en el conjunto de prueba transformado
